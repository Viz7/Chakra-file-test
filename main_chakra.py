...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=10, shuffle=True, pin_memory=False)

    for train_data in train_loader:
        print(len(train_data))
        # batched_data=[d.tolist() for d in train_data]
        # batched_data=torch.tensor(batched_data).to(device)
