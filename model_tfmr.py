...
class TfmrAttention(nn.Module):
    ...
    def _attn(self, query, key, value):
        ...
        causal_mask = self.bias[ : , : , key.shape[-2] - query.shape[-2] : key.shape[-2], : key.shape[-2]].bool()
...
class TfmrBlock(nn.Module):
    ...
    def forward(
        ...
        ffn_outputs = self.mlp(hidden_states)
        hidden_states = residual + ffn_outputs
...
class TfmrModel(nn.Module):
    ...
    def forward(
        ...
        position_id = torch.arange(past_length, past_length + input_shape[-1], dtype = int, device = device).unsqueeze(0)
...
class TfmrLMHeadModel(nn.Module):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            mask = mask[ : , : -1]
            loss = ((element_loss * mask).sum(dim = 1) / mask.sum(dim = 1)).mean()
            ...
            for i in range(0, int(5000/batch_size)+1):
                ...
                for _ in range(maxlen):
                    ...
                    if decode_strategy == "top-p":
                        ...
                        sorted_prob, sorted_id = prob.sort()
                        cumsum = sorted_prob.cumsum(dim = 1)
                        mask = torch.zeros_like(cumsum, dtype=bool).scatter(1, sorted_id, cumsum <= 1 - top_p)
